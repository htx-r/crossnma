crossnma.code <- function(ipd = TRUE,
  ...
  ## Construct default priors following
  # basic paramaters
  d.prior <- sprintf("\nfor (k in 2:nt) {d[k] ~ dnorm(0, (%s*15)^(-2))}",max.d)
  ...
  p.b[j] ~ dunif(0, 1)}"
    ...
    prior.u <-sprintf("for (j in 1:(ns.ipd + ns.ad)) {u[j] ~ dnorm(0, (%s*15)^(-2))}",max.d)
  }
  ## heterogienty parameters
  prior.tau.trt <- replaceNULL(prior.tau.trt, sprintf("dunif(0,%s)", max.d))
  prior.tau.reg0 <- replaceNULL(prior.tau.reg0, sprintf("dunif(0,%s)", max.d))
  prior.tau.regb <- replaceNULL(prior.tau.regb, sprintf("dunif(0,%s)", max.d))
  prior.tau.regw <- replaceNULL(prior.tau.regw, sprintf("dunif(0,%s)", max.d))
  prior.tau.gamma<- replaceNULL(prior.tau.gamma, sprintf("dunif(0,%s)", max.d))
...
for (k in 2:nt) {beta.t_", i, "[k]", sprintf("~ dnorm(0, (%s*15)^(-2))",max.d),"}")
          ##
...
betab.t_", i, "[1] <- 0
for (k in 2:nt) {betab.t_", i, "[k]", sprintf("~ dnorm(0, (%s*15)^(-2))",max.d),"}")
          ##
...
betab.t_", i, "[1] <- 0
for (k in 2:nt) {betab.t_", i, "[k] <- bb_", i, "}
bb_", i, sprintf("~ dnorm(0, (%s*15)^(-2))",max.d))
            ##
            betab.prior <- paste0(betab.prior, betab.prior0)
          }
       ...
       paste0("\n# Random effect for betab (the between-study covariate effect)
betaw.t_", i, "[1] <- 0
for (k in 2:nt) {betaw.t_", i, "[k]", sprintf("~ dnorm(0, (%s*15)^(-2))",max.d))
          ##
...
betaw.t_", i, "[1] <- 0
for (k in 2:nt) {betaw.t_", i, "[k] <- bw_", i, "}
bw_", i, sprintf("~ dnorm(0, (%s*15)^(-2))",max.d))
            ##
            betaw.prior.ipd <- paste0(betaw.prior.ipd, betaw.prior.ipd0)
          }
...
"\n prec.beta_", i, " <- pow(tau.b_", i, ", -2)")
          ...
          for (i in 1:length(covariate[[1]])) {
            beta.prior.ad0 <-
              paste0("\n# Common effect for beta (within=between)
betab.t_", i, "[1] <- 0
for (k in 2:nt) {betab.t_", i, "[k] <- b_", i, "}
b_", i, sprintf("~ dnorm(0, (%s*15)^(-2))",max.d))
            ##
            beta.prior.ad <- paste0(beta.prior.ad, beta.prior.ad0)
          }
          ...
          for (i in 1:2) {
            gamma.effect0 <-
              paste0("\n# Random effect for gamma (bias effect)",
                     ...
                     ifelse(add.std.in, paste0("\ng", i, sprintf("~ dnorm(0, (%s*15)^(-2))",max.d)), ""),
                     ifelse(add.std.act.yes, paste0("\ng.act", i, sprintf("~ dnorm(0, (%s*15)^(-2))",max.d)), ""),
...
"\n b",sprintf("~ dnorm(0, (%s*15)^(-2))",max.d)
  ...
  link.str.ad <-"logit(pa[j,t.ad[j,k]]) <- u[j+ns.ipd]+(theta[j+ns.ipd,t.ad[j,k]])"
    ...
    like.str.ad <- " r[j,k] ~ dbin(pa[j,t.ad[j,k]],n[j,k]) # binomial likelihood of number of events"
    link.str.ad.ref <- "log(pa[j,t.ad[j,1]]) <- u[j+ns.ipd]   # Log of risk probability at referent arm"
    link.str.ad <-"log(pa[j,t.ad[j,k]]) <- u[j+ns.ipd]+(theta[j+ns.ipd,t.ad[j,k]])"

  } else if(sm=="MD") {
  like.str.ipd <- " y[i]~dnorm(delta[i],prec.delta.ipd[study[i],trt.index[i]])  # normal likelihood "
  link.str.ipd <- "delta[i] <- u[study[i]]+(theta[study[i],trt[i]])"

  like.str.ad <- " ybar[j,k] ~ dnorm(delta.ad[j,t.ad[j,k]],prec.delta.ad[j,k]) # normal likelihood"
  link.str.ad.ref <- "delta.ad[j,t.ad[j,1]] <- u[j+ns.ipd] # Mean outcome at referent arm"
  link.str.ad <-"delta.ad[j,t.ad[j,k]] <- u[j+ns.ipd]+(theta[j+ns.ipd,t.ad[j,k]])"

  }else if(sm=="SMD") {
  like.str.ipd <- " y[i]~dnorm(phi[i],prec.delta.ipd[study[i],trt.index[i]]) # normal likelihood"
  link.str.ipd <- "phi[i] <- delta[i]*s.pool.ipd[study[i]]
                   delta[i] <- u[study[i]]+theta[study[i],trt[i]]"

  like.str.ad <- " ybar[j,k] ~ dnorm(phi.ad[j,t.ad[j,k]],prec.delta.ad[j,k]) # normal likelihood
