...
```{r}
knitr::kable(summary(jagsfit1,exp=F))
```
...
In this part, we run NMR model by adding age as a covariate from both datasets. We set `covariate='age'` representing the name of the covariate column in `prt.data` and `std.data`.
...
league_table <- crossnma.league(jagsfit2,exp = T, prt.cov.value = 38,cell.text.size = 4.5)
...
```{r}
knitr::kable(summary(jagsfit3,exp=F))
```
...
```{r}
knitr::kable(summary(jagsfit4,exp=F))
```
...
```{r}
knitr::kable(summary(jagsfit5,exp=F))
```
