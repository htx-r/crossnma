...
#---------- bias adjustment ----------
method.bias = "adjust1"
bias="bias" #optional, required for adjust1. It can be either: low, high,
unfav="unfav"
bias.type="add" #c('add','mult','both'),
bias.group="bias.group"
bias.covariate=NULL
bias.effect='common'
# ---------- prior ----------
prior=list(pi.high.rct=NULL,
           pi.low.rct=NULL
)
...
study.df <- data.frame(std.id= c(unique(data11$study),unique(data22$study)))
